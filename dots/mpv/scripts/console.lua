...
local function get_matches_to_print(terminal)
	...
	local highlight = terminal and terminal_styles.matched_position
		or "{\\1c&H" .. option_color_to_ass(opts.match_color) .. "}"
...
local function render()
	...
	for i, item in ipairs(items) do
		...
		if
			(
				first_match_to_print - 1 + i == selected_match
		...
		then
			...
			ass:an(4)
			ass:pos(x, item_y)
			ass:append("{\\blur0\\bord0\\4aH&ff&\\1c&H" .. option_color_to_ass(opts.selected_back_color) .. "&}")
...
local function move_history(amount, is_wheel)
	...
	if is_wheel then
		...
		if
			(amount > 0 and selected_match == first_match_to_print and first_match_to_print - 1 + max_lines < #matches)
			or (amount < 0 and selected_match == first_match_to_print - 1 + max_lines and first_match_to_print > 1)
		then
